...
// ===== Middleware Setup =====
// Configure CORS - Updated to match render.yaml configuration
const ALLOWED_ORIGINS = new Set([
  'http://localhost:3000',
  'http://localhost:5173',
  'https://sagradago.onrender.com',
  'http://sagradago.onrender.com',
  'https://sagradago.online',
  'https://www.sagradago.online',
  'https://sagradago.netlify.app',
  'https://sagradago-backend.onrender.com',
  'https://sagradago-frontend.onrender.com'  // Added for render deployment
]);
...
const startServer = async () => {
  try {
    ...
    const optionalEnvVars = ['RECAPTCHA_SITE_KEY', 'RECAPTCHA_SECRET_KEY'];
    const missingOptionalVars = optionalEnvVars.filter(varName => !process.env[varName]);
    if (missingOptionalVars.length > 0) {
      console.warn('⚠️  WARNING: Optional environment variables missing:', missingOptionalVars.join(', '));
